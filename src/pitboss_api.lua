--[[
  ...
  Disclaimer: Reverse engineering for interoperability. Third-party driver not endorsed by Pit Boss or Dansons. Use at your own risk.
...
local function codec(data_str, key_list, paddingLen, is_rpc_auth_or_status)
    ...
    if paddingLen > 0 then
        ...
        for _ = 1, paddingLen do
            ...
            if rndv == 0xff then rndv = 0xfe end
...
local function parse_url(url_string)
    local protocol, host, port, path = "http", nil, tonumber(80), "/"
    ...
    if port_start then
        ...
        port = tonumber(host_port_segment:sub(port_start + 1))
    else
...
local function get_auth_data(ip_address)
    ...
    if auth_cache.password and 
       auth_cache.timestamp > 0 then
...
local function convert_temperature(byte_array, offset)
    ...
    if (hundreds == 0 and tens == 9 and units == 6) or
       (hundreds == 0 and tens == 0 and units == 0) or
       (hundreds == 255 and tens == 255 and units == 255) then
        return "Disconnected"
    end

    local temp_val = (hundreds * 100) + (tens * 10) + units
    return temp_val == 960 and "Disconnected" or temp_val
end
...
local function parse_grill_status(sc_11_bytes, sc_12_bytes)
    ...
    if sc_12_bytes and #sc_12_bytes >= 27 then
        ...
        status.p1_target = "Disconnected"
        status.p1_temp = "Disconnected"
        status.p2_temp = "Disconnected"
        status.p3_temp = "Disconnected"
        status.p4_temp = "Disconnected"
        status.set_temp = "Disconnected"
        status.grill_temp = "Disconnected"
    end
    ...
    if sc_11_bytes and #sc_11_bytes >= 24 then
        ...
        status.smoker_temp = "Disconnected"
...
local function post_with_auth(url, initial_payload, time_int_val, psw_hex1, psw_hex2)
...
function pitboss_api.get_status(ip_address)
    ...
    local response, err = post_with_auth(url, payload, time_int, psw_hex, psw_hex_plus1)
    if err or not response then
        return nil, err
    end
...
function pitboss_api.set_temperature(ip_address, temperature)
    if not (type(temperature) == "number" and temperature >= 0 and temperature <= 999) then
