...
-- Helper: centralize panic log prefix
local function panic_log_prefix()
...
function panic_manager.update_last_active_time(device)
  ...
  log.debug(string.format("Updated last active time to: %d", current_time))
end
...
local function was_grill_recently_active(device)
  ...
  if last_active_time <= 0 then
    log.debug("No last active time recorded, treating as not recently active")
    return false
  end
  ...
  if time_since_active < 0 then
    log.warn(string.format("Negative time difference detected (current=%d, last_active=%d), resetting last_active_time", 
      current_time, last_active_time))
    device:set_field("last_active_time", current_time, {persist = true})
    return false
  end
  
  log.debug(string.format("Panic check: last_active=%d, current=%d, time_since=%d, threshold=%d", 
    last_active_time, current_time, time_since_active, config.CONSTANTS.PANIC_TIMEOUT))
...
local function update_panic_display(device)
  ...
  log.debug(string.format("Updated panic display: panic_state=%s", tostring(panic_state)))
end
