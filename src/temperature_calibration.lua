...
function calibration.apply_calibration(raw_temp, offset, unit, sensor_name)
  ...
  log.info(string.format(
    ...
    temp_diff_from_ref,
    scaling_factor,
    calibrated_temp, unit
  ))
  
  return calibrated_temp
end

--- Get calibration description for documentation
-- @return string Description of the calibration method
function calibration.get_calibration_description()
  return [[
Temperature Calibration Information:

The temperature offset values represent the difference from a 32°F (0°C) ice water reference point.
For example, if your probe reads 35°F when placed in ice water, you should set the offset to -3°F.

This driver uses the Steinhart-Hart equation for thermistor-based temperature calibration:
1. Calculates thermistor resistance from raw temperature using Steinhart-Hart equation
2. Applies calibration offset to determine corrected resistance values
3. Uses corrected resistance to calculate final calibrated temperature
4. Maintains thermistor non-linear response characteristics across full temperature range

The Steinhart-Hart equation: 1/T = 1/T₀ + (1/B) × ln(R/R₀)
Where: T = temperature (Kelvin), B = thermistor beta coefficient, R = resistance, R₀ = reference resistance

To calibrate your probes:
1. Prepare an ice bath (crushed ice + water, stirred well)
2. Insert probe and wait for temperature to stabilize
3. Note the reading and calculate difference from 32°F (0°C)
4. Enter the negative of this difference as your offset

Example: Probe reads 36°F in ice water → Set offset to -4°F
]]
end
