...
function handlers.switch_handler(driver, device, command)
  ...
  local success = command_service.send_power_command(device, driver, state)
...
function handlers.thermostat_setpoint_handler(driver, device, command)
  ...
  log.info(string.format("Thermostat setpoint command: %s°C for device: %s", 
    requested_celsius_setpoint, device.id))

  local success = command_service.send_temperature_command(device, driver, requested_celsius_setpoint)
...
function handlers.light_control_handler(driver, device, command)
  ...
  if not state then
    ...
    device_status_service.set_status_message(device, "Invalid light command")
    return
  end
  
  local success = command_service.send_light_command(device, driver, state)
...
function handlers.prime_control_handler(driver, device, command)
  ...
  if not state then
    ...
    device_status_service.set_status_message(device, "Invalid prime command")
    return
  end
  
  local success = command_service.send_prime_command(device, driver, state)
...
function handlers.temperature_unit_handler(driver, device, command)
  ...
  if not unit_arg then
    ...
    device_status_service.set_status_message(device, "Invalid unit command")
    return
  end
  
  local success = command_service.send_unit_command(device, driver, unit_arg)
...
function handlers.virtual_thermostat_handler(driver, device, command)
  ...
  log.info(string.format("Virtual thermostat command for %s: setpoint=%s", parent_key, tostring(command.args.setpoint)))
...
handlers.capability_handlers = {
  ...
  [capabilities.thermostatHeatingSetpoint.ID] = {
    [capabilities.thermostatHeatingSetpoint.commands.setHeatingSetpoint.NAME] = function(driver, device, command)
      route_or_call(device, handlers.virtual_thermostat_handler, handlers.thermostat_setpoint_handler, driver, device, command)
    end
  },
