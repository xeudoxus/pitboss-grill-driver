...
local function require_grill_on(device, operation_name)
  ...
  if not device_status_service.is_grill_on(device) and ip_preference ~= config.CONSTANTS.DEBUG_IP_ADDRESS then
    ...
    device_status_service.set_status_message(device, operation_name .. " failed (Grill Off)")
    return false
  end
  return true
end
...
local function update_virtual_device_immediately(device, virtual_key, state)
  ...
  for _, virtual_device in ipairs(driver:get_devices()) do
    if virtual_device.parent_device_id == device.id and 
       virtual_device.parent_assigned_child_key == virtual_key then
  ...
  virtual_device:emit_event(capabilities.switch.switch[state]())
      log.debug(string.format("Updated %s immediately: %s", virtual_key, state))
      break
    end
  end
end
...
function command_service.send_power_command(device, driver, state)
  ...
  if not require_grill_on(device, "Grill Power On") then
    ...
    local success = network_utils.send_command(device, "set_power", state, driver)
...
-- @return boolean True if command was sent successfully
function command_service.send_temperature_command(device, driver, requested_celsius_setpoint)
  ...
  if not require_grill_on(device, "Set temp") then
    ...
    device:emit_event(custom_caps.grillTemp.targetTemp({value = config.CONSTANTS.DISCONNECT_DISPLAY, unit = device_display_unit}))
    
    local standard_grill = device.profile.components[config.COMPONENTS.GRILL]
    device:emit_component_event(standard_grill, capabilities.thermostatHeatingSetpoint.heatingSetpoint({value = min_temp, unit = device_display_unit}))
  ...
  if not temperature_service.is_valid_setpoint(requested_celsius_setpoint, "C") then
    ...
    log.error(string.format("Invalid input temperature setpoint: %s°C. Range: %s-%s°C", 
      requested_celsius_setpoint, temp_range.min, temp_range.max))
    device_status_service.set_status_message(device, "Invalid input temperature")
    return false
  end
  ...
  if device_display_unit == "C" then
    ...
    log.debug(string.format("Snapped input %sÂ°C to %sÂ°C for Celsius grill", requested_celsius_setpoint, snapped_temp_for_display))
  else
    ...
    temp_to_send_to_grill = snapped_temp_for_display
    log.debug(string.format("Converted %sÂ°C to %sÂ°F, snapped to %sÂ°F for Fahrenheit grill", 
      requested_celsius_setpoint, requested_fahrenheit_for_snapping, snapped_temp_for_display))
  end
  ...
  if not temperature_service.is_valid_setpoint(temp_to_send_to_grill, device_display_unit) then
    ...
    log.error(string.format("Invalid temperature setpoint after snapping: %s°%s. Range: %s-%s°%s", 
      temp_to_send_to_grill, device_display_unit, temp_range.min, temp_range.max, device_display_unit))
    device_status_service.set_status_message(device, "Invalid temperature range after snapping")
    return false
  end

  log.debug(string.format("Final temperature to send: %sÂ°%s", temp_to_send_to_grill, device_display_unit))
  
  local success = network_utils.send_command(device, "set_temperature", temp_to_send_to_grill, driver)
  
  if success then
    log.info(string.format("Temperature setpoint sent successfully: %sÂ°%s", snapped_temp_for_display, device_display_unit))
    ...
    if cached_target then
      ...
      if math.abs(snapped_temp_for_display - cached_target) >= threshold then
        ...
        log.debug(string.format("Target temp changed significantly (%d to %d) - reset session tracking", cached_target, snapped_temp_for_display))
      end
    end
    
    -- Update UI immediately with snapped temperature for responsive feedback
    device:emit_event(custom_caps.grillTemp.targetTemp({value = string.format("%.0f", snapped_temp_for_display), unit = device_display_unit}))
    
    local standard_grill = device.profile.components[config.COMPONENTS.GRILL]
    device:emit_component_event(standard_grill, capabilities.thermostatHeatingSetpoint.heatingSetpoint({value = snapped_temp_for_display, unit = device_display_unit}))
    
    return true
  else
    -- Network failure - set generic error message
    device_status_service.set_status_message(device, "Failed to set temp")
    return false
  end
end
...
-- @return boolean True if command was sent successfully
function command_service.send_light_command(device, driver, state)
  ...
  local success = network_utils.send_command(device, "set_light", api_state, driver)
  ...
  if success then
    ...
    -- Network failure - set generic error message
    device_status_service.set_status_message(device, "Failed to control light")
    return false
  end
end
...
-- @return boolean True if command was sent successfully
function command_service.send_prime_command(device, driver, state)
  ...
  local success = network_utils.send_command(device, "set_prime", api_state, driver)
  ...
  if success then
    ...
    if state == "ON" then
      ...
      local timer_ref = device.thread:call_with_delay(config.CONSTANTS.PRIME_TIMEOUT, function()
        ...
        network_utils.send_command(device, "set_prime", "off", driver)
      end)
      device:set_field("prime_auto_off_timer", timer_ref)
      
      -- Update status to show priming
      device_status_service.set_status_message(device, "Connected (Grill Priming)")
    else
      ...
      device_status_service.set_status_message(device, "Connected (Grill Prime Off)")
      ...
      device.thread:call_with_delay(3, function()
        ...
        if current_status and current_status.value == "Connected (Grill Prime Off)" then
    ...
    -- Network failure - set generic error message
    device_status_service.set_status_message(device, "Failed to control prime")
    return false
  end
end
...
-- @return boolean True if command was sent successfully
function command_service.send_unit_command(device, driver, unit_arg)
  ...
  local success = network_utils.send_command(device, "set_unit", api_unit, driver)
  ...
  if success then
    ...
    -- Network failure - set generic error message
    device_status_service.set_status_message(device, "Failed to change temperature unit")
    return false
  end
end
