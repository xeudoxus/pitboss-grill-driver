...
function network_utils.update_device_ip(device, new_ip)
  if not device or not new_ip or new_ip == "" then return false end
...
function network_utils.resolve_device_ip(device, retest)
  ...
  if pref_ip and pref_ip ~= config.CONSTANTS.DEFAULT_IP_ADDRESS and pref_ip ~= config.CONSTANTS.DEBUG_IP_ADDRESS then
    table.insert(candidates, {ip = pref_ip, source = "Preference", priority = 1})
  end
  ...
  log.debug(string.format("Found %d IP candidates for validation (device: %s)", #candidates, device.id))
  ...
  for i, c in ipairs(candidates) do
    ...
    if not valid then
      ...
      local cache_ok = cache and cache.device_id == device.id and 
                      (os.time() - cache.timestamp) < (config.CONSTANTS.IP_CACHE_VALIDATION_MINUTES * 60)
  ...
  log.warn(string.format("No valid IP address found for device %s after testing %d candidates", 
           device.id, #candidates))
  return nil
end

--- Manually trigger cache cleanup (can be called externally)
-- @return number Number of entries cleaned
function network_utils.cleanup_cache()
  return cleanup_ip_cache()
end

--- Get cache statistics for monitoring
-- @return table Cache statistics
function network_utils.get_cache_stats()
  local validated_count = 0
  local metadata_count = 0
  local expired_count = 0
  local current_time = os.time()
  
  for _, entry in pairs(ip_management_cache.validated_ips) do
    validated_count = validated_count + 1
    if (current_time - entry.timestamp) > (config.CONSTANTS.IP_CACHE_VALIDATION_MINUTES * 60) then
  ...
  return {
    validated_ips = validated_count,
    expired_ips = expired_count,
    metadata_entries = metadata_count,
    last_cleanup = ip_management_cache.last_cleanup,
    next_cleanup_due = ip_management_cache.last_cleanup + config.CONSTANTS.IP_CACHE_CLEANUP_INTERVAL
  }
end
...
function network_utils.find_device_by_network_id(driver, network_id)
  ...
  if not network_id or network_id == "" then return nil end
...
function network_utils.build_device_profile(grill_data)
  if not grill_data or not grill_data.id or not grill_data.ip then return nil end
  local metadata = {
    ip = grill_data.ip,
...
function network_utils.validate_ip_address(ip)
  ...
  if a < 1 or a > 255 or b < 0 or b > 255 or c < 0 or c > 255 or d < 0 or d > 255 then
    log.debug(string.format("IP validation failed: Invalid segment ranges - %d.%d.%d.%d", a, b, c, d))
    return false, "IP address segments must be between 0-255 (first segment 1-255)"
  end
...
function network_utils.is_rediscovery_ip(ip)
  return ip == config.CONSTANTS.DEFAULT_IP_ADDRESS or ip == config.CONSTANTS.DEBUG_IP_ADDRESS
end
...
-- @param device SmartThings device object
-- @param driver SmartThings driver object (unused but kept for compatibility)
-- @return boolean True if device is reachable and responding
function network_utils.health_check(device, driver)
  ...
  -- As a final fallback, consider preference even if default (legacy behavior) only if nothing resolved
  if (not ip or ip == "") and device.preferences then
    local pref_ip = device.preferences.ipAddress or device.preferences["ipAddress"]
    if pref_ip and pref_ip ~= "" then
      ip = pref_ip
      log.debug("Using fallback preference IP for health check: " .. tostring(ip))
    end
  end
  ...
  if sys_info then
    ...
    log.warn(string.format("Health check failed for %s after %ds: %s", 
             ip, elapsed_time, err or "unknown error"))
    return false
  end
end
...
function network_utils.test_grill_at_ip(ip, device_network_id_to_match)
  ...
  if sys_info and type(sys_info) == "table" and sys_info.app == config.CONSTANTS.PITBOSS_APP_IDENTIFIER then
    ...
    if firmware_version then
      ...
      log.info(string.format("Grill firmware version: %s (valid: %s)", firmware_version, tostring(firmware_valid)))
    else
    ...
    if device_network_id_to_match then
      if sys_info.id == device_network_id_to_match then
        log.info(string.format("Grill FOUND and MATCHED at IP: %s (ID: %s, FW: %s, response time: %ds)", 
                 ip, sys_info.id, firmware_version, elapsed_time))
        sys_info.ip = ip
        sys_info.firmware_version = firmware_version
        sys_info.firmware_valid = firmware_valid
        return sys_info
      else
        log.debug(string.format("Grill found at %s but ID %s does not match expected ID %s", 
                  ip, sys_info.id or "unknown", device_network_id_to_match))
        return nil
      end
    else
      -- No ID matching required, return any Pit Boss grill found
      log.info(string.format("Grill DISCOVERED at IP: %s (ID: %s, FW: %s, response time: %ds)", 
               ip, sys_info.id or "unknown", firmware_version, elapsed_time))
      sys_info.ip = ip
      sys_info.firmware_version = firmware_version
      sys_info.firmware_valid = firmware_valid
      return sys_info
...
function network_utils.scan_for_grills(driver, subnet, start_ip, end_ip, callback)
  ...
  local function run_next()
    ...
    cosock.spawn(function()
      ...
      if success and grill_data then
        ...
        -- Optional: Cancel remaining scans after finding first grill (uncomment if desired)
        -- scan_cancelled = true
        -- queue = {}
  ...
  log.info(string.format("Starting balanced grill scan with max %d concurrent connections: %s.%d to %s.%d (%d total)",
    max_concurrent, subnet, start_ip, subnet, end_ip, total_ips))

  -- Start initial batch
  for i = 1, math.min(max_concurrent, #queue) do
    run_next()
  end
end
...
function network_utils.rediscover_device(device, driver)
  -- Enhanced flood protection: Prevent multiple concurrent scans
  ...
  if scan_in_progress and (current_time - scan_start_time) > 300 then -- 5 minutes max
    log.warn(string.format("Detected stale rediscovery flag (running for %ds) - force resetting", 
  ...
  if scan_in_progress then
    ...
    log.info(string.format("Rediscovery already in progress (%ds elapsed) - ignoring request to prevent flooding", elapsed))
    return false
  end
  ...
  -- Build IP queue
  for i = config.CONSTANTS.DEFAULT_SCAN_START_IP, config.CONSTANTS.DEFAULT_SCAN_END_IP do
    table.insert(queue, string.format("%s.%d", subnet, i))
  end
  
  local total_ips = #queue
  log.info(string.format("Starting rediscovery scan: %d IPs with max %d concurrent connections", 
           total_ips, max_concurrent))
  ...
  local function safe_cleanup_and_return(result, reason)
    ...
    log.info(string.format("Rediscovery completed: %s (%s, %d/%d IPs, %ds)", 
             tostring(result), reason, completed, total_ips, elapsed))
    ...
    cosock.spawn(function()
      ...
      if success and grill_data and not found_device and not cleanup_called then
        ...
        if device_network_id and grill_data.id and grill_data.id ~= device_network_id then
          ...
          log.debug(string.format("Found grill at %s but ID mismatch: %s != %s", 
                    ip, grill_data.id, device_network_id))
        end
...
function network_utils.send_command(device, command, args, driver)
  local ip = network_utils.resolve_device_ip(device, false)
  if not ip or ip == "" then
    log.error(string.format("No valid IP address available for command '%s' (device: %s)", 
              command, device.id))
    return false
  end
  ...
  if not network_utils.health_check(device, driver) then
    log.warn("Device health check failed before command, attempting recovery")
    device:offline()
    pcall(function() device:set_field("is_connected", false, {persist = true}) end)
    
    -- Attempt rediscovery and recovery if preferences allow
    if network_utils.should_attempt_rediscovery(device) then
      log.info("Attempting rediscovery for command recovery")
      if network_utils.rediscover_device(device, driver) then
        if network_utils.health_check(device, driver) then
          device:online()
          pcall(function() device:set_field("is_connected", true, {persist = true}) end)
          log.info("Device recovered successfully, proceeding with command")
          -- Update IP after rediscovery
          ip = network_utils.resolve_device_ip(device, false)
        else
          log.error("Device rediscovered but still failing health check")
          return false
        end
      else
        log.error("Device rediscovery failed, cannot send command")
        return false
      end
    else
      log.debug("Rediscovery skipped due to preferences, command cannot be sent")
      return false
    end
  end
  ...
  repeat
    ...
    if success then
      log.info(string.format("Command '%s' sent successfully%s", command, 
               retry_count > 0 and string.format(" (after %d retries)", retry_count) or ""))
      return true
    else
      retry_count = retry_count + 1
      log.warn(string.format("Command '%s' failed (attempt %d/%d): %s", 
               command, retry_count, config.CONSTANTS.COMMAND_RETRY_COUNT + 1, err or "unknown error"))
...
function network_utils.get_status(device, driver)
  ...
  if not network_utils.health_check(device, driver) then
    ...
    -- Only attempt rediscovery if preferences allow it
    if network_utils.should_attempt_rediscovery(device) then
      log.info("Attempting rediscovery based on preferences")
      
      -- Attempt rediscovery (protection is now handled inside rediscover_device function)
      if network_utils.rediscover_device(device, driver) then
        if network_utils.health_check(device, driver) then
          device:online()
          pcall(function() device:set_field("is_connected", true, {persist = true}) end)
          log.info("Device rediscovered, proceeding with status request")
          -- Update IP after successful rediscovery
          ip = network_utils.resolve_device_ip(device, false)
        else
          log.error("Failed to rediscover device")
          return nil
        end
      else
        log.error("Rediscovery failed")
        return nil
      end
    else
      log.debug("Rediscovery skipped due to preferences")
      return nil
    end
  end
