...
function network_utils.update_device_ip(device, driver, new_ip)
	...
	if current and current ~= new_ip then
		...
		if driver then
			network_utils.health_check(device, driver)
		else
			log.warn("No driver available to perform health check after IP update")
		end
	end
	return true
end
...
function network_utils.resolve_device_ip(device, retest)
	...
	-- Preference is explicitly unset/blank
		...
		cosock.spawn(function()
			...
			if success and grill_data and not found_device and not cleanup_called then
				...
				if is_target_device then
					...
					network_utils.update_device_ip(device, driver, ip)
