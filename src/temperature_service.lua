--[[
  ...
  - Caching: Temporary storage with timeout-based invalidation
  - Snapping: Alignment to approved Pit Boss setpoint values
  - Display: Formatted output with unit indicators and disconnect handling
...
local function should_use_cached_value(device, field_name)
  ...
  if time_since_update < 0 then
    log.warn(string.format("Negative time difference detected for %s cache (current=%d, last_update=%d), invalidating cache", 
      field_name, current_time, last_update_time))
  ...
  if should_cache then
    log.debug(string.format("Using cached value for %s (age: %ds, timeout: %ds)", field_name, time_since_update, cache_timeout))
  else
    log.debug(string.format("Cache expired for %s (age: %ds, timeout: %ds)", field_name, time_since_update, cache_timeout))
  end
...
function temperature_service.get_cached_temperature_value(device, field_name, default_value)
  if not device then
    log.debug(string.format("get_cached_temperature_value called with nil device for field '%s' - returning default", tostring(field_name)))
...
function temperature_service.clear_temperature_cache(device)
  local cache_fields = {"grill_temp", "set_temp", "p1_temp", "p2_temp"}
...
function temperature_service.track_session_temp_reached(device, current_temp, target_temp)
  ...
  -- Check if target temperature has changed significantly
  ...
  if currently_at_temp and not session_reached_temp then
    ...
    log.debug(string.format("Session reached target temp: current=%.1f >= threshold=%.1f", current_num or 0, temp_threshold))
    return true
  end
...
function temperature_service.is_grill_preheating(device, runtime, current_temp, target_temp)
  ...
  local is_preheating = (not session_reached_temp) and (type(current_temp) == "number" and current_temp < temp_threshold)

  log.debug(string.format("Preheating check: current=%s, target=%s, threshold=%.1f, session_reached=%s, preheating=%s", 
    tostring(current_temp), tostring(target_temp), temp_threshold, tostring(session_reached_temp), tostring(is_preheating)))
...
function temperature_service.is_grill_heating(device, current_temp, target_temp)
  ...
  log.debug(string.format("Heating check: current=%s, target=%s, threshold=%.1f, session_reached=%s, heating=%s", 
    tostring(current_temp), tostring(target_temp), temp_threshold, tostring(session_reached_temp), tostring(is_heating)))

  -- Only consider this a 'heating' event if the session has previously reached the target
  return is_heating and session_reached_temp
end
