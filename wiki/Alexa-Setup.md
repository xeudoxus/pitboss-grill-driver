...
| --------------------------- | ---------------------------------------- | -------------------------------------- |
| **Virtual Grill Main**      | Switch only (power control)              | _"Alexa, turn off the grill"_          |
| **Virtual Grill Light**     | Switch control                           | _"Alexa, turn on the grill light"_     |
| **Virtual Grill Probe 1/2** | Full temperature sensor (visual + voice) | _"Alexa, what's probe 1 temperature?"_ |
| **Virtual Grill Prime**     | Switch control                           | _"Alexa, turn on grill prime"_         |
| **Virtual Grill At-Temp**   | Switch status                            | _"Alexa, is the grill at temp?"_       |
| **Virtual Grill Error**     | Switch status                            | _"Alexa, check grill status"_          |

> **💡 Pro Tip**: Create Alexa routines (see Step 4) to make voice commands more natural:
>
> - _"Alexa, prime the grill"_ → turns on Grill Prime
> - _"Alexa, is the grill ready?"_ → checks if At-Temp is on
...
<a href="images/alexa-sync-noti.jpg"><img src="images/alexa-sync-noti.jpg" alt="Alexa automatic device notification" width="400"></a>

_Alexa automatically notifies you when new devices are detected (click to enlarge)._

**Manual Discovery (Optional)**:

1. **Say**: _"Alexa, discover my devices"_
...
3. **Rename each device** with clear, voice-friendly names:
   - `Grill` or `Pit Boss Grill` (main power switch) _Off Only_
   - `Grill Light` (light control)
   - `Grill Prime` (pellet prime)

### Virtual Devices in Alexa

<a href="images/virtual-in-alexa.png"><img src="images/virtual-in-alexa.png" alt="Virtual devices displayed in Alexa app" width="600"></a>

_How the virtual grill devices appear in the Amazon Alexa app after discovery (click to enlarge)._
...
3. **When**: Voice trigger _"Start grilling"_
4. **Actions**:
...
3. **Try alternative phrasings**:
   - _"Turn off THE grill"_ vs _"Turn off grill"_
   - _"Grill light"_ vs _"Light"_
...
- **Be specific**: _"Turn on the grill light"_ vs _"Turn on light"_
- **Use room names**: _"Turn off the patio grill"_ (if assigned to rooms)
