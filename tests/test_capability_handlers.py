...
class TestCapabilityHandlers(LuaTestBase):
    ...
    def setUp(self):
        ...
        if package.loaded["command_service"] then
            ...
            package.loaded["command_service"].send_temperature_command = function(device, driver, temp)
            ...
            package.loaded["command_service"].send_light_command = function(device, driver, state)
            ...
            package.loaded["command_service"].send_prime_command = function(device, driver, state)
            ...
            package.loaded["command_service"].send_unit_command = function(device, driver, unit)
            ...
            package.loaded["command_service"].send_power_command = function(device, driver, state)
                table.insert(command_sent, {type = "power", value = state})
                return true
            end

            -- Mock schedule_refresh to avoid calling refresh_device
            package.loaded["command_service"].schedule_refresh = function(device, driver, command)
    ...
    def test_debug_services(self):
        ...
        cs.send_temperature_command(self.lua_device, self.mock_driver, 123)
        ...
        rs.refresh_device(self.lua_device, self.mock_driver, {"command": "refresh"})
